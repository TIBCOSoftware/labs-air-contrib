...
import (
	...
	"github.com/SteveNY-Tibco/labs-air-contrib/connector/graph"
...
func (a *Activity) Eval(context activity.Context) (done bool, err error) {
	logCache.Info("[BuilderActivity:Eval] entering ........ ")
	...
	deltaGraph, err = a.graphMgr.CreateGraph(input.Nodes, input.Edges, a.allowNullKey)
	...
	if input.BatchEnd {
		...
		if 0 != len(passThroughDataDef) {
			logCache.Info("[BuilderActivity:Eval] PassThroughData : ", context.GetInput(PassThroughData))
			passThroughData := context.GetInput(PassThroughData).(map[string]interface{})
	...
	logCache.Info("[BuilderActivity:Eval] Exit ........ ")

	return true, nil
}

func (a *Activity) buildPassThroughData(myId string, context activity.Context) map[string]*Field {
