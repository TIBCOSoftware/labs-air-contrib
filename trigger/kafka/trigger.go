...
func (h *Handler) consumePartition(consumer sarama.PartitionConsumer) {
	for {
		...
		case msg := <-consumer.Messages():
			...
			case "Base64":
				byteContent, err := base64.StdEncoding.DecodeString(string(msg.Value))
				if err != nil {
					h.logger.Errorf("Run action for handler [%s] failed for reason [%s] message lost", h.handler.Name(), err)
					return
				}
			...
			case "Base64JSON":
				decodedByteContent, err := base64.StdEncoding.DecodeString(string(msg.Value))
				if err != nil {
					h.logger.Errorf("Run action for handler [%s] failed for reason [%s] message lost", h.handler.Name(), err)
					return
				}
